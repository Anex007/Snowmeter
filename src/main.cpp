...
constexpr std::array<float, NUM_BARS> barney_starsoup_cant_make_shit()
{
	std::array<float, NUM_BARS> barney_the_big_retard = {};
	for (int i = 0; i < NUM_BARS; i++)
		barney_the_big_retard[i] = 1.0f;
	return barney_the_big_retard;
}
...
static float* audio_engine_getbars(const float* audio_data)
{
	...
	static auto maxHeights = barney_starsoup_cant_make_shit();
...
int main()
{
	...
	while (window.Running()) {
		...
		// glClearColor(0.26f, 0.29f, 0.26f, 1.0f);
